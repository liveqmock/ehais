...
//		String ret = null;
		// 如果账号密码正确
		try{
			ReturnObject<HaiUsers> ro = loginService.login(userName, password);
			if(ro.getCode()!=1){
	...
	public String registerSubmit(HttpServletRequest request, 
		...
		try {
			ReturnObject<HaiUsers> ro = loginService.register(userName,email, password, confirmPassword, verificationCode,code);
