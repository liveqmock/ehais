...
@EPermissionController(name="公众号配置",intro="微信公众号管理功能",value="wpPublicCEOController")
@Controller
@RequestMapping("/ceo")
public class  WpPublicCEOController extends CommonController {

	private static Logger log = LoggerFactory.getLogger(WpPublicCEOController.class);
	...
	public String wpPublicDetail(ModelMap modelMap,
		...
		try{
			...
			modelMap.addAttribute("rm", rm);
			return "/admin/public/public_detail";
		}catch(Exception e){
			e.printStackTrace();
			log.error("public", e);
			return this.errorJump(modelMap, e.getMessage());
	...
	@ResponseBody
	@EPermissionMethod(intro="更新微信公众号数据缓存",value="wpPublicCache",type=PermissionProtocol.JSON)
	@RequestMapping(value="/wpPublicCache",method=RequestMethod.POST,produces={"application/json;charset=UTF-8"})
	public String wpPublicCache(ModelMap modelMap,
			HttpServletRequest request,HttpServletResponse response,
			@ModelAttribute EConditionObject condition,
			@RequestParam(value = "publicId", required = true) Integer publicId) {
		try{
			ReturnObject<WpPublicWithBLOBs> rm = wpPublicService.public_cache(request, publicId);
			return this.writeJson(rm);
		}catch(Exception e){
			e.printStackTrace();
			log.error("public", e);
