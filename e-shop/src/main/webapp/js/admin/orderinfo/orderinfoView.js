var haiOrderInfoModal ;
var key_orderName = "";
var validform = null;
$(function(){
	haiOrderInfoModal = $("#haiOrderInfoModal").modal({backdrop: 'static', keyboard: false , show : false });
	
	$("#haiOrderInfoAddDetail").click(function(){
		haiOrderInfoAddDetail();
	});
	
	$("#haiOrderInfoSearch").click(function(){
		key_orderName = $.trim($("#key_orderName").val());
		bsTable.bootstrapTable('refresh', { query : { orderName : key_orderName , page : 1} });
	});
    
	
	$("#haiOrderInfoSaveSubmit").click(function(){$("#haiOrderInfoForm").attr("action") == "add" ? haiOrderInfoAddSubmit() : haiOrderInfoEditSubmit() ;});

	validform = $("#haiOrderInfoForm").validate({rules:{},messages:{}});
	
});

function haiOrderInfoAddDetail(){
	haiOrderInfoModal.modal("show");
	$("#haiOrderInfoForm").attr("action","add");
	$('#haiOrderInfoForm')[0].reset();
	select_reset_parentId();
}


function haiOrderInfoEditDetail(orderId){
	$("#haiOrderInfoForm").attr("action","edit");
	layer.load(0, {shade: false});
	$.ajax({
		url : "haiOrderInfoEditDetail",type:"post",dataType:"json",data:{orderId:orderId},
