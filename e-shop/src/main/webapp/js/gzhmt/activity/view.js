...
{
    field: 'activityName',
    title: '会议名称',formatter:function(value,rows,index){
    	if(rows.level > 0){
...
function addCate(){
	layer.prompt({title: '请输入会议管理分类名称', formType: 0}, function(text, index){
	    
		if(text == null || text == ""){
			layer.msg("请输入会议管理分类名称");return ;
		}
		$.ajax({
...
function editCate(){
	...
	var catId = node[0].id;
	layer.prompt({title: '请输入会议管理分类名称', formType: 0 ,value : node[0].text}, function(text, index){
