...
$(function(){
    ...
    bsTable = $('#bsTable').bootstrapTable({
        ...
        url: 'ehaisBrandStoryListJson',//请求后台的url
        method: 'post',//请求方式
			...
			},
		{
            ...
            formatter : function(value,row,index){
            	...
            	var a = "&nbsp;|&nbsp;<a href ='ehaisBrandStoryEditDetail?articleId="+value+"'>编辑</a>";
            	var b = "&nbsp;|&nbsp;<a href ='javascript:;' onclick='ehaisBrandStoryDelete("+value+");' >删除</a>";
            	return c+a+b;
            }
        }
...
function getTree() {
	$.ajax({
		url : "ehaisBrandStoryCatListJson",type:"post",dataType:"json",data:{},
...
function ehaisBrandStoryDelete(articleId){
	layer.confirm('您确定要删除此项吗？',{
		btn: ['确定删除','不删除'] //按钮
	}, function(){
		$.ajax({
			url : "ehaisBrandStoryDelete",type:"post",dataType:"json",data:{articleId:articleId},
			success:function(result){
...
function addCate(){
	layer.prompt({title: '请输入分类名称', formType: 0}, function(text, index){
		...
		$.ajax({
			url : "ehaisBrandStoryCatAddSubmit",
			data  : {catName : text},
			success : function(result){
				if(result.code != 1){
					layer.msg(result.msg);
...
function editCate(){
	...
	layer.prompt({title: '请输入分类名称', formType: 0 ,value : node[0].text}, function(text, index){
		...
		$.ajax({
			url : "ehaisBrandStoryCatEditSubmit",
...
function deleteCate(){
	...
	layer.confirm('您确定要删除此项吗？',{
		...
		$.ajax({
			url : "ehaisBrandStoryCatDelete",data:{catId:id},
			success:function(result){
				layer.msg(result.msg);
				if(result.code != 1){
					return ;
