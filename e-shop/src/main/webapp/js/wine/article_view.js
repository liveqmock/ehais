...
$(function(){
    ...
    bsTable = $('#bsTable').bootstrapTable({
        ...
        url: 'wineArticleListJson',//请求后台的url
        method: 'post',//请求方式
			...
			},
		{
            ...
            formatter : function(value,row,index){
            	var a = "<a href ='wineArticleEditDetail?articleId="+value+"'>编辑</a>";
            	var b = "&nbsp;|&nbsp;<a href ='javascript:;' onclick='wineArticleDelete("+value+");' >删除</a>";
            	return a+b;
            }
        }
...
function getTree() {
	$.ajax({
		url : "wineArticleCatListJson",type:"post",dataType:"json",data:{},
...
function wineArticleDelete(articleId){
	layer.confirm('您确定要删除此项吗？',{
		btn: ['确定删除','不删除'] //按钮
	}, function(){
		$.ajax({
			url : "wineArticleDelete",type:"post",dataType:"json",data:{articleId:articleId},
			success:function(result){
...
function deleteCate(){
	...
	layer.confirm('您确定要删除此项吗？',{
		...
		$.ajax({
			url : "wineArticleCatDelete",data:{catId:id},
			success:function(result){
				layer.msg(result.msg);
				if(result.code != 1){
					return ;
