...
function getTree() {
	$.ajax({
		url : "wineCategoryListJson",type:"post",dataType:"json",data:{},
...
function deleteCate(){
	...
	layer.confirm('您确定要删除此项吗？',{
		...
		$.ajax({
			url : "wineCategoryDelete",data:{catId:catId},
			success:function(result){
				layer.msg(result.msg);
				if(result.code != 1){
					return ;
