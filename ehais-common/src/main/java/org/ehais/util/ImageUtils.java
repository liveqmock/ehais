...
public class ImageUtils {
	 ...
	 * 
	 * // 5-给图片添加文字水印： // 方法一：
	 * ImageUtils.pressText("我是水印文字","e:/abc.jpg","e:/abc_pressText.jpg","宋体",
	 * Font.BOLD,Color.white,80, 0, 0, 0.5f);//测试OK // 方法二：
	...
	public final static void cut(String srcImageFile, String result, int x, int y, int width, int height) {
		try {
			...
			if (srcWidth > 0 && srcHeight > 0) {
				...
				ImageFilter cropFilter = new CropImageFilter(x, y, width, height);// 用于裁剪图像的
																					// ImageFilter
																					// 类。此类扩展了基本
																					// ImageFilter
																					// 类，可提取现有
																					// Image
																					// 中的给定矩形区域，为包含刚提取区域的新图像提供源。也就是它要与
																					// FilteredImageSource
																					// 对象结合使用，以生成现有图像的裁剪版本。
	...
	public static void main(String[] args) {
		try {
			InputStream orgImg = EHttpClientUtil.getUriStream("https://oss.dinghuo123.com/notify/images/aa1ea534-8f3f-4457-a8cd-51a41522e34d.png");
			InputStream morImg = EHttpClientUtil.getUriStream("http://pic.qianmi.com/qmui/v0.1/img/pro-login-logo.png");
			InputStream midImg = EHttpClientUtil.getUriStream("https://sso.dinghuo123.com/portal/images/logo-min.png");
			
			BufferedImage biOrgImg =ImageIO.read(orgImg);    
			BufferedImage biMorImg =ImageIO.read(morImg);
			BufferedImage biMidImg =ImageIO.read(midImg);
			...
			int morImgWidth = biMorImg.getWidth();// 获取层图的宽度
			int morImgHeight = biMorImg.getHeight();// 获取层图的高度
			System.out.println(morImgWidth+"==="+morImgHeight);
			
			int midImgWidth = biMidImg.getWidth();// 获取层图的宽度
			int midImgHeight = biMidImg.getHeight();// 获取层图的高度
			System.out.println(midImgWidth+"==="+midImgHeight);
			
			
			g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1f));//SRC_ATOP
			
			// 绘制
			g2d.drawImage(biMorImg, (orgImgWidth - morImgWidth ) / 2 , (orgImgHeight - morImgHeight) / 2, morImgWidth, morImgHeight, null);
			g2d.drawImage(biMidImg, 10 , 10, midImgWidth, midImgHeight, null);
			
			g2d.setColor(Color.BLACK);
			g2d.setFont(new Font(null, 0, 16));
			g2d.drawString("姓名：梁tyler", 40, 60);
			g2d.drawString("手机号：234987498345", 40, 80);
			g2d.drawString("职务：skdkkkkk", 40, 100);
