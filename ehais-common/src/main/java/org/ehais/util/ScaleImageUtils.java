...
//	        return resize(size[0], size[1], savePath, image);
	...
	public static void scaleImageJPG(int width, int height,String inputFilePath, String outputFilePath) throws IOException {  
	    BufferedImage buffered_oldImage = ImageIO.read(new File(inputFilePath));  
	    int imageOldWidth = buffered_oldImage.getWidth();  
	    int imageOldHeight = buffered_oldImage.getHeight();  
	    double scale_x = (double) width / imageOldWidth;  
	    double scale_y = (double) height / imageOldHeight;  
...
//        JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(fos);
//        encoder.encode(buffered_newImage);
//  
//        buffered_newImage.flush();
//        fos.flush();
//        fos.close();
     ...
     *  
     */ 
	public static void scaleImage(int width,int height,String fromFileStr, String saveToFileStr//, String sysimgfile
			,String suffix
			) throws Exception {  
        ...
        File ThF = new File(saveToFileStr);  //new File(saveToFileStr, sysimgfile +"."+suffix)
        BufferedImage Bi = ImageIO.read(F);  
        int imageOldWidth = Bi.getWidth();  
	    int imageOldHeight = Bi.getHeight();  
//	    System.out.println(imageOldWidth + "  ==  " + imageOldHeight);
        Image Itemp = Bi.getScaledInstance(width, height, Bi.SCALE_SMOOTH);  
        if ((imageOldHeight > width) || (imageOldWidth > height)) {  
            if (Bi.getHeight() > Bi.getWidth())  
                Ratio = Double.valueOf(width)  / Double.valueOf(imageOldHeight);  
            else  
                Ratio = Double.valueOf(height) / Double.valueOf(imageOldWidth);  
        }  
...
//		 ScaleImageUtils.scaleImage(180,180,
//				 "E:/LGJ/icon.png",
//				 "E:/LGJ/80_80.png");
		 try {
			ScaleImageUtils.scaleImage(80,80,
					 "E:/LGJ/icon.png",
					 "E:/LGJ/80_80.png","png");
		} catch (Exception e) {
