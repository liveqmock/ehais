...
@Component
public class EUniqueValid implements ConstraintValidator<EUnique, String> {
    ...
    public boolean isValid(String str, ConstraintValidatorContext constraintValidatorContext) {    
    	...
    	if(str != null && !str.equals("")){
        	int c = eCommonMapper.commonUniqueStore(tableName, fieldName, str,(Integer)request.getSession().getAttribute(EConstants.SESSION_STORE_ID) );
            if(c <= 0){    
                return true;    
            }else{
            	return false;
